...
---------------------------------------------------
  通过"实例.属性"得到值时，顺序如下:
    ...
    2,__getattribute__将"实例.属性"转换为 type(实例).__dict__('属性').__get__(实例,type(实例)) 查找数据描述符
    3,__getattribute__将"实例.属性"转换为 实例.__dict__('属性') 查找本实例中是否有此属性
    4,__getattribute__将"实例.属性"转换为 type(实例).__dict__('属性').__get__(实例,type(实例)) 查找非数据描述符
    5,通过 __getattr__ 方法查找是否有此属性，没有则抛出异常
