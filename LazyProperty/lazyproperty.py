...
(上面本来是想用"def area_t(self) ...... area = LazyProperty(area_t)"的，但是这样写了之后用t.arae发现print还是会打印,后来发现是本描述符在setattr时会把值绑定到函数名的【同名】变量上，即要调用t.area_t才行，所以又改回了函数名)
---------------------------------------------------
描述符说明:
---------------------------------------------------
  通过"实例.属性"得到值时，顺序如下:
    ...
    6,查找父类的类属性中是否有此属性
    7,通过 __getattr__ 方法查找是否有此属性，没有则抛出异常
